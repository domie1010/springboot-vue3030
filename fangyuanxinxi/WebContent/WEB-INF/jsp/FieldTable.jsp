...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/fieldAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("field/informationLoadOne?id="+id);
	        }
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${pager.datas }" var="field" varStatus="vg">
						<tr>
							<td><c:out value="${field['name'] }"></c:out></td>
							<td><c:out value="${field['area']['name'] }"></c:out></td>
							<td><c:out value="${field['area']['city']['name'] }"></c:out></td>
							<td><c:out value="${field['createtime'] }"></c:out></td>
							<td>
							<a href="javascript:;" onclick="update(${field['id']})"
								class="tablelink">查看</a> 
							<a href="javascript:;" onclick="deletecheck(${field['id']})" class="tablelink">删除</a></td>
						</tr>
					</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="field/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="field/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="field/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="field/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="field/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="field/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
