...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/houseBaseAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("houseBase/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="houseBase/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="imgtable">
				...
				<tbody>
					...
					<c:forEach items="${houseBaseList.datas }" var="houseBase" varStatus="vg">
						<tr>
							<td>
								<div id="images">
								<img src="upload/upload/${houseBase['url'] }">
								</div>
							</td>
							<td><c:out value="${houseBase['name']}"></c:out></td>
							<td><c:out value="${houseBase['address']}"></c:out></td>
							<td>
							<c:if test="${houseBase['video'] eq null or houseBase['video'] eq ''}" var="t1">
								<a href="request/videoAdd?id=${houseBase['id'] }" class="tablelink">上传视频</a>
							</c:if>
							<c:if test="${!t1}">
								<a href="upload/upload/${houseBase['video'] }" class="tablelink">查看视频</a>
							</c:if>
							</td>
							<td><c:out value="${houseBase['createtime']}"></c:out></td>
							<td>
							<a href="javascript:;" onclick="update(${houseBase['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${houseBase['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="houseBase/informationLoadBuy?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="houseBase/informationLoadBuy?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="houseBase/informationLoadBuy?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="houseBase/informationLoadBuy?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="houseBase/informationLoadBuy?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="houseBase/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
