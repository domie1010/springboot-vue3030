				...
				<tbody>
					<c:forEach items="${messageList.datas }" var="message" varStatus="vg">
						<tr>
							...
							<td><c:out value="${message['house']['name'] }"></c:out></td>
							<td><c:out value="${message['createtime'] }"></c:out></td>
							<td>
								<c:if test="${message['flag'] eq 0}">
									<c:out value="未回复" />
								</c:if>
								<c:if test="${message['flag'] eq 1}">
									<c:out value="已回复" />
								</c:if>
							</td>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="message/informationLoad?pageNow=${pager.pageNow }&doctor_id=${sessionScope['Doctor']['id']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="message/informationLoad?pageNow=${pager.pageNow-1 }&doctor_id=${sessionScope['Doctor']['id']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="message/informationLoad?pageNow=${vs.count }&doctor_id=${sessionScope['Doctor']['id']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="message/informationLoad?pageNow=${count }&doctor_id=${sessionScope['Doctor']['id']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="message/informationLoad?pageNow=${count }&doctor_id=${sessionScope['Doctor']['id']}">${count}</a></li>
