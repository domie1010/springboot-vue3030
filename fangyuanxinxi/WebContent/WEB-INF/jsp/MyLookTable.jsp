		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${lookList.datas }" var="look" varStatus="vg">
						<tr>
							<td><c:out value="${look['userInfo']['userName'] }"></c:out></td>
							<td><c:out value="${look['name'] }"></c:out></td>
							<td><c:out value="${look['house']['name'] }"></c:out></td>
							<td><c:out value="${look['look_time'] }"></c:out></td>
							<td><c:out value="${look['result'] }"></c:out></td>
						</tr>
					</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="look/informationLoadMy?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="look/informationLoadMy?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="look/informationLoadMy?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="look/informationLoadMy?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="look/informationLoadMy?pageNow=${count }">${count}</a></li>
