...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/houseRentAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("house/informationLoadOneRent?id="+id);
	        }
...
<body>
	...
	<form action="house/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="imgtable">
				...
				<tbody>
					...
					<c:forEach items="${houseList.datas }" var="house" varStatus="vg">
						<tr>
							<td>
								<div id="images">
								<img src="upload/upload/${house['url'] }">
								</div>
							</td>
							<td><c:out value="${house['name']}"></c:out></td>
							<td><c:out value="${house['address']}"></c:out></td>
							<td><c:out value="${house['houseBase']['name']}"></c:out></td>
							<td><c:out value="${house['size']}"></c:out></td>
							<td><c:out value="${house['price']}"></c:out></td>
							<td><c:out value="${house['owner']}"></c:out></td>
							<td><c:out value="${house['createtime']}"></c:out></td>
							<td><c:if test="${house['showw'] eq 0}" var="s1">
							<a href="house/informationUpdateStatus?showw=1&id=${house['id'] }"
								class="tablelink">展示</a>
							</c:if>
							<c:if test="${!s1 }">
								<a href="house/informationUpdateStatus?showw=0&id=${house['id'] }"
								class="tablelink">隐藏</a>
							</c:if>
							<c:if test="${house['good'] eq 0}" var="s2">
								<a href="house/informationUpdateStatus?good=1&id=${house['id'] }"
								class="tablelink">精品</a>
							</c:if>
							<c:if test="${!s2}">
								<a href="house/informationUpdateStatus?good=0&id=${house['id'] }"
								class="tablelink">取消精品</a>
							</c:if>
							<a href="javascript:;" onclick="update(${house['id']})"
								class="tablelink">查看</a>
							<a href="javascript:;"
								onclick="deletecheck(${house['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="house/informationLoadRent?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="house/informationLoadRent?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="house/informationLoadRent?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="house/informationLoadRent?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="house/informationLoadRent?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="house/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
