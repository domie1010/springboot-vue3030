			...
			<div id="tab1" class="tabson">
				...
				<form action="user/userUpdate" method="POST"
					...
					<ul class="forminfo">
						...
						<c:if test="${sessionScope['Admin']['roleInfo']['id'] eq 1}">
						<li><label>经纪人星级<b>*</b></label><div class="vocation">
						<c:if test="${message!=null }">
								<c:out value="${message }"></c:out>
							</c:if> <select class="select3" name="selfId" id="selfId">
								<c:forEach items="${self }" var="self">
									<c:if test="${self['id'] eq user['self']['id']}" var="s1">
										<option value="${self['id'] }" selected="selected">${self['name'] }</option>
									</c:if>
									<c:if test="${!s1}">
						...
						<li><label>个人信息<b>*</b></label><textarea id="content7" name="remark">${user['remark'] }</textarea></li>
