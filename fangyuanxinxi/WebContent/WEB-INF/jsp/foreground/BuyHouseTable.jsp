...
<script type=text/javascript src="js/js/foreground/houset.js"></script>
...
<div class="shopquery">
		<div class="fang" style="width:120px;height:120px">
			...
			<a href="house/informationLoadOneForeBuy?id=${house['id'] }" target="_self" ><img src="upload/upload/${house['url'] }" style="width:120px;height:120px" border="0" /></a>
			</div>
		</div>
		<div class="prop">
		价格：<span class="hb">￥</span><span class="price">${house['price'] }</span><br />
		</div>
		<a href="house/informationLoadOneForeBuy?id=${house['id'] }" target="_self" class="prodtitle"   >${house['name'] }</a>
		<div class="memo" >
		${house['name'] }<c:if test="${house['good'] eq 1}"><font color="red">(精品房源)</font></c:if>
		</div>
</div>
</c:forEach>
</div>
...
<div id="showpages" >
	  ...
	  <a href="house/informationLoadForeBuy?pageNow=${pager.pageNow }">上一页</a>
	  </c:if>
	   <c:if test="${!pagele}">
	  <a href="house/informationLoadForeBuy?pageNow=${pager.pageNow -1}">上一页</a>
	  </c:if>
	  </li>
	  <li class="pbutton">
	  <c:if test="${pager.pageNow ge pager.totalPages }" var="pagege">
	  <a href="house/informationLoadForeBuy?pageNow=${pager.pageNow }">
	  下一页
	  </a>
	  </c:if>
	  <c:if test="${!pagege }">
	  <a href="house/informationLoadForeBuy?pageNow=${pager.pageNow+1 }">
	  下一页
	  </a>
	  </c:if>
	  </li></ul></div>
